        ...
        function saveOngoingGame() {
            ...
            // pencilMarks contains Sets — serialize them to arrays
        ...
        function showOngoingBanner(snapshot) {
            const banner = document.getElementById('ongoing-game-banner');
            if (!banner) return;

            const elapsed = Math.floor((Date.now() - snapshot.savedAt) / 1000);
            const mins = Math.floor(elapsed / 60);
            const secs = elapsed % 60;
            const timeAgo = mins > 0 ? `${mins}m ago` : `${secs}s ago`;

            const modeLabel = snapshot.gameMode === 'passplay' ? 'Pass & Play' :
                              snapshot.gameMode === 'simultaneous' ? 'Simultaneous' : 'Solo';
            const vsLabel = snapshot.vsAI
                ? `vs ${CONFIG.AI_DIFFICULTY[snapshot.aiDifficulty]?.name || 'AI'}`
                : snapshot.onlineRoomId ? 'Online' : 'Local';

            document.getElementById('ongoing-score').textContent =
                `${snapshot.scores.p1} – ${snapshot.scores.p2}`;
            document.getElementById('ongoing-meta').textContent =
                `${modeLabel} · ${vsLabel} · left ${timeAgo}`;

            // Mini progress bar: % of cells claimed
            const totalCells = 81;
            const claimed = snapshot.claims.flat().filter(c => c !== 0).length;
            const pct = Math.round((claimed / totalCells) * 100);
            document.getElementById('ongoing-progress-fill').style.width = pct + '%';
            document.getElementById('ongoing-progress-pct').textContent = pct + '%';

            banner.classList.add('visible');
        }

        function hideOngoingBanner() {
            const banner = document.getElementById('ongoing-game-banner');
        ...
        function checkAndShowOngoingBanner() {
            const s = loadOngoingGame();
            if (s) showOngoingBanner(s);
            else hideOngoingBanner();
        }

        function exitToLobby() {
            // Pause but SAVE the game — don't destroy it
            saveOngoingGame();
            gameState.isRunning = false;
        ...
        function init() {
            ...
            // Ongoing game banner
            checkAndShowOngoingBanner();
            document.getElementById('ongoing-resume-btn').addEventListener('click', resumeGame);
            document.getElementById('ongoing-dismiss-btn').addEventListener('click', () => {
                clearOngoingGame();
                hideOngoingBanner();
            });
